...
rem {Copyright}
rem {License}
rem Сценарий копирования ADF-библиотек приложений потоков задач (Adf-библиотеках проектов ViewController) по расписанию
rem согласно итерациям разработки в среду TEST
...
if "%p_key%" EQU "-fd" set from_dir=%p_value%
if "%p_key%" EQU "-td" set to_dir=%p_value%
if "%p_key%" EQU "-sf" set schedule_file=%p_value%
...
if "%from_dir%" EQU "" call :exec_format %module_name% & endlocal & exit /b 1
if "%to_dir%" EQU "" call :exec_format %module_name% & endlocal & exit /b 1
if "%schedule_file%" EQU "" call :exec_format %module_name% & endlocal & exit /b 1

if not exist "%schedule_file%" (
	rem ChangeColor 12 0 
	%ChangeColor_12_0%
	echo ERROR-01: Не найден файл расписания: %schedule_file%^!
 	call :exec_format %module_name% & endlocal & exit /b 1
)
if not exist "%from_dir%" (
	rem ChangeColor 12 0 
	%ChangeColor_12_0%
	echo ERROR-02: Не найден исходный каталог: %from_dir%^!
 	call :exec_format %module_name% & endlocal & exit /b 1
)
if not exist "%to_dir%" (
	rem ChangeColor 12 0 
	%ChangeColor_12_0%
	echo ERROR-02: Не найден каталог назначения: %from_dir%^!
 	call :exec_format %module_name% & endlocal & exit /b 1
)

rem получаем учётные данные WLS
for /F "usebackq eol=; tokens=1,2,3 delims=	" %%i in ("%schedule_file%") do (
	if /i "%WLS_HOST%" EQU "%%i" (
		set WLS_USER_NAME=%%j
		set WLS_PASSWORD=%%k
	)
...
rem ChangeColor 11 0
%ChangeColor_11_0%
echo | set /p "dummyName=   -fd"
rem ChangeColor 15 0
%ChangeColor_15_0%
echo | set /p "dummyName= :(обязательно) "
rem ChangeColor 8 0
%ChangeColor_8_0%
echo абсолютный путь к исходному каталогу, содержащему ADF-библиотеки
rem ChangeColor 11 0
%ChangeColor_11_0%
...
rem ChangeColor 8 0
%ChangeColor_8_0%
echo абсолютный путь к каталогу назначения
rem ChangeColor 11 0
%ChangeColor_11_0%
echo | set /p "dummyName=   -sf"
rem ChangeColor 15 0
%ChangeColor_15_0%
echo | set /p "dummyName= :(обязательно) "
rem ChangeColor 8 0
%ChangeColor_8_0%
echo абсолютный путь к файлу расписания [содержит строки вида: имя_файла_архива_adf_библиотеки дата_начала_разработки длительность_итерации_в_днях]
exit /b 0
rem ---------------- EOF adflib_copy.cmd ----------------