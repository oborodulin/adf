...
set OHW_base_URI_DEV=dc00-wlsdap-01.metinvest.ua:7001
set OHW_base_URI_TEST=dc00-wlstap-01.metinvest.ua:7001
...
if /i "%oper%" EQU "delete" (
	...
	%ChangeColor_8_0%
	echo -----------------------------------------------------------
	FOR /f "tokens=1,2,3,4,5" %%a IN ('2^>nul FORFILES /S /p "%deploy_dir%" /m "*.ear" /C "cmd /c echo @path @file @fdate_@ftime @relpath"') DO (
		set ear_path=%%~a
		set ear_file=%%~b
